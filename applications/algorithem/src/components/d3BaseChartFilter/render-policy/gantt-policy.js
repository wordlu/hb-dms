...
export default class PhBarChartPolicy extends policy{
    ...
    render (d3, data, ele) {
        ...
        const dataArray = data.map(item => {
          const itemArray = item.filter(it => it[that.yProperty] && (it[that.yProperty].toLowerCase() !== 'fn' && it[that.yProperty].toLowerCase() !== 'fp'))
          const specialItem = item.filter(it => it[that.yProperty] && (it[that.yProperty].toLowerCase() === 'fn' || it[that.yProperty].toLowerCase() === 'fp'))[0]
          if (specialItem) itemArray.push(specialItem)
          return itemArray
        })
        const dataArraySort = dataArray.filter(it => it[0] && it[0][that.xProperty] && (it[0][that.xProperty].toLowerCase() !== 'fn' && it[0][that.xProperty].toLowerCase() !== 'fp'))
        const dataArraySpecialItem = dataArray.filter(it => it[0] && it[0][that.xProperty] && (it[0][that.xProperty].toLowerCase() === 'fn' || it[0][that.xProperty].toLowerCase() === 'fp'))[0]
        if (dataArraySpecialItem) dataArraySort.push(dataArraySpecialItem)
        console.log("最终数据", dataArraySort)

        const labelsDatax = dataArraySort[0] ?  dataArraySort[0].map(it => it[that.yProperty]) : []
        labelsDatax.unshift("")
        const labelsDatay = dataArraySort.map(it => it[0] ? it[0][that.xProperty] : '')
        labelsDatay.unshift("")
        console.log(labelsDatax, labelsDatay)
        const legned = ["正确"]

        const numrows = dataArraySort.length;
        const numcols = dataArraySort[0] ? dataArraySort[0].length : 0
        ...
        // Compute values.
        const X = d3.map(dataArraySort, computeX);
        const Y = d3.map(dataArraySort, computeY);
        ...
        if (title === undefined) {
            ...
            const O = d3.map(dataArraySort, d => d);
            const T = title;
            title = i => T(O[i], i, dataArraySort);
        }
        ...
        cell.append("text")
            ...
            .style("fill",(d, x, y) => {
              if (d[that.xProperty] === d[that.yProperty]) return '#ffffff'
              return '#000000'
            })
            .text(function(d, i) { return d[that.zProperty]; });
        
        row.selectAll(".cell")
            .data(function(d, i) { return dataArraySort[i]; })
            .style("fill", (d, x, y) => {
              if (d[that.xProperty] === d[that.yProperty]) return '#50BBAE'
              return '#ffffff'
            })
        ...
        const legneds = labels.selectAll(".legned")
            ...
            .enter().append("g")
            .attr("class", "legned")
            .attr("transform", function(d, i) { return "translate(" + 0  + "," + (y.bandwidth() * (dataArraySort.length)) + ")"; });

        legneds.append('circle')
            .attr('r', 7)
            .attr('cy', (v, i) => { return i*20+5 })
            .attr("fill", "#50BBAE")
            .attr("transform", function(d, i) { return "translate(" + ((x.bandwidth() * (labelsDatax.length - 1) - marginLeft) / 2 - marginLeft - d.length * 9)  + "," + 14 + ")"; });
        
        legneds.append("text")
            .attr("x", (d, i) => {
              return (x.bandwidth() * (labelsDatax.length - 1) - marginLeft) / 2 - marginLeft
            })
