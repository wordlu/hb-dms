<template>
  <div class="container">
    <div class="panel">
      <div class="mid-panel">
        <el-input v-model="search" class="search-bar" :placeholder="$t(`algorithm['搜索版本号']`)" @change="change" @input="change"
          :prefix-icon="Search" />
        <div class="mid-group">
          <div class="ver-mid">
          ...
          <div class="ver-mid">
            <el-button type="info" class="info-btn" plain :disabled="!currentRow.id" @click="remove"
    ...
    <div class="list">
      <div class="list-panel">
        <el-table ref="multipleTableRef" :data="tableData" style="width: 100%">
          ...
          <el-table-column property="name" :label="$t(`algorithm['版本号']`)" width="300">
            <template #default="scope">
...
const remove = () => {
  ElMessageBox.confirm(
    ...
    deleteItem('algorithmversions', params).then(res => {
      ElMessage({
...
const rowClicked = (row: Row) => {
  window.history.pushState(null, '', `/algorithem/detail/${props.algorithem}/version/${row.id}?name=${props.name}`)
}
