...
// @ts-ignore 
import { ArrowRight, Search, ArrowRightBold, ArrowLeftBold, Delete } from "@element-plus/icons-vue" // eslint-disable-line
...
const remove = () => {
  ElMessageBox.confirm(
    ...
    deleteItem('environmentversions', params).then(res => {
      ElMessage({
...
const rowClicked = (row: Row) => {
  window.history.pushState(null, '', `/algorithem/env/${props.envId}/version/${row.id}?name=${props.name}`)
}
...
const getList = async (page: number) => {
  const params = {
    ...
    environment: props.envId,
  }
  findAll('Environmentversion', params).then(res => {
    ...
    tableData.value = Environmentversion
  })
}
