      ...
      <div class="project-list-content">
        ...
        <div class="list-header">
          <div class="left">
            ...
            <span class="list-header-title">{{$t(`Menu['项目列表']`)}}</span>
          </div>
          <!-- 操作按钮 -->
          <div class="right">
        ...
        <div class="list-container">
          <div v-for="(item,index) in projectList"  class="project-item" :key="index">
            ...
            <div class="projectHomeMore">
              <el-icon><Download @click="deployCancelProject(item)"/></el-icon>
            </div>
          </div>
        ...
        <el-empty :description="$t(`common['暂无数据']`)" v-if="projectList.length === 0" :image-size="200"></el-empty>
      </div>
    </div>
  </div>
</template>
...
import { fetchComponentsAction, autoDeploymentPut } from '@/api/flow';
...
//发布项目
const deployCancelProject = (data) => {
  ElMessageBox.alert( t(`dataprojects['是否要撤销已发布的项目']`), t(`common['系统提示']`), { 
    showCancelButton: true,
    confirmButtonText: t(`common['确认']`),
    cancelButtonText: t(`common['取消']`),
  }).then(() => {
    deployCancelProjectSubmit(data)
  })
  .catch((err) => {
    console.log(err)
  })
...
const deployCancelProjectSubmit = (data) => {
  const paramsValue = {
    "project": data.id,
    "action": "unrelease"
  }
  autoDeploymentPut(paramsValue).then(res=>{
    ElMessage({
      message:  t(`dataprojects['撤销发布项目成功']`),
      type: 'success',
    })
    getList()
  }).catch((err) => {
    console.log(err)
    ElMessage({
      message: t(`dataprojects['撤销发布项目失败']`),
      type: 'error',
    })
  })
}
